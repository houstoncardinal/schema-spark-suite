...
import { FileText, Loader2, ArrowRight, Brain, BookOpen, Sparkles, CheckCircle, AlertTriangle } from "lucide-react";
...
export function ContentAnalyzer() {
  ...
  const analyze = async () => {
    ...
    try {
      // Step 1: Scrape real page content
    ...
    <div>
      ...
      {loading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card-float p-16 text-center">
          ...
          <p className="font-medium text-foreground">Scraping and analyzing real content...</p>
          <p className="text-sm text-muted-foreground mt-1">Crawling page, evaluating semantic relevance, readability, and content depth</p>
        </motion.div>
      )}
