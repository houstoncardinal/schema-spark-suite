...
import { Search, Loader2, ArrowRight, TrendingUp, TrendingDown } from "lucide-react";
...
export function KeywordResearchTool() {
  ...
  const search = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setResults({
        main: { keyword, volume: 12400, difficulty: 67, cpc: 4.50, intent: "Commercial" },
        trend: [
          { month: "Jul", volume: 9800 }, { month: "Aug", volume: 10200 }, { month: "Sep", volume: 11400 },
          { month: "Oct", volume: 12100 }, { month: "Nov", volume: 12800 }, { month: "Dec", volume: 11900 },
          { month: "Jan", volume: 12400 }, { month: "Feb", volume: 13100 }, { month: "Mar", volume: 14200 },
        ],
        suggestions: [
          ...
          { keyword: `${keyword} agency`, volume: 2100, difficulty: 58, cpc: 7.20, trend: "up" },
          { keyword: `free ${keyword}`, volume: 5400, difficulty: 42, cpc: 1.50, trend: "up" },
        ],
      });
    }, 2000);
  };
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: "Difficulty", value: `${results.main.difficulty}/100`, sub: "Competitive" },
          ...
          <div className="glass-card-float p-6">
            <h3 className="font-display text-sm font-semibold text-foreground mb-4">Search Volume Trend</h3>
                ...
                <tbody>
                  {results.suggestions.map((s: any) => (
                    <tr key={s.keyword} className="border-b border-border/50 hover:bg-secondary/50 transition-colors">
                      ...
                      <td className="py-3 text-right">
                        {s.trend === "up" ? <TrendingUp className="h-4 w-4 text-success ml-auto" /> : <TrendingDown className="h-4 w-4 text-destructive ml-auto" />}
                      </td>
                    </tr>
                  ))}
                </tbody>
